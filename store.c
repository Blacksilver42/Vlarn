...
{ 150,	potionname,	OPOTION,	23,	3	} ,	/* 60 */
...
{ 500,	scrollname,	OSCROLL,	16,	2	} , /* 80 */
...
lprcat("Sorry, but it seems you are trying to pay off your taxes by cheating!");
	...
	while (1) {
		...
		case 'p':
			...
			if (amt<0) {
			lprcat("  Sorry, but we can't take negative gold");
				amt=0;
			}
			else {
				if (amt>c[GOLD])
