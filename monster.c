...
ws:
	...
	case 6:
		i = rnd(3) + 2;
		s = "  While the %s is entangled, you hit %d times";
		goto ws;	/* web */

	case 7:
		if (c[LARNSTRCOUNT] == 0)
			c[STREXTRA] += 3;	/* strength */
		c[LARNSTRCOUNT] += 150 + rnd(100);
		return;
	...
	int             spnum;
{
	...
	while (monster[m = mitem[x][y].mon = rnd(MAXMONST + 7)].genocided);
...
spout:
	...
	case 10:
		p = "\nThe %s hit you with it's barbed tail";
		i = rnd(25) - c[AC];
		goto spout2;
...
void genmonst()
{
	...
	for (j = 0; j < MAXMONST; j++)	/* search for the monster type */
		...
		if (monster[j].genocided == 0 || wizard) {
